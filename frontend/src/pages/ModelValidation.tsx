  ...
  return (
    <div style={{ display: 'flex', gap: '24px', flex: 1 }}>
      ...
      <div style={{ width: '40%', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        <div style={{ backgroundColor: '#111927', padding: '24px', borderRadius: '20px', border: '2px solid rgba(255,255,255,0.1)' }}>
          <h3 style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 900, fontFamily: 'Orbitron', margin: '0 0 20px' }}>
            OVERALL PERFORMANCE
          </h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            {[
              ...
              <div key={metric.label} style={{ padding: '16px', borderRadius: '12px', backgroundColor: 'rgba(0,0,0,0.3)' }}>
                <span style={{ color: '#6B7280', fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase' }}>{metric.label}</span>
        ...
        <div style={{ backgroundColor: '#111927', padding: '24px', borderRadius: '20px', border: '2px solid rgba(255,255,255,0.1)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
            ...
            <h3 style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 900, fontFamily: 'Orbitron', margin: 0 }}>EVALUATION PROTOCOL</h3>
          </div>
      ...
      <div style={{ width: '60%', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <thead>
              <tr>
                {['Class', 'Precision', 'Recall', 'F1-Score', 'Support'].map(h => (
                  <th key={h} style={{ color: '#6B7280', fontSize: '0.65rem', fontWeight: 700, padding: '8px 12px', textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {perClass.map(c => (
                <tr key={c.className}>
                  <td style={{ color: '#fff', fontSize: '0.75rem', fontWeight: 700, padding: '10px 12px' }}>{c.className}</td>
                  ...
                  <td style={{ color: '#9CA3AF', fontSize: '0.75rem', fontFamily: 'monospace', padding: '10px 12px' }}>{c.support}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {/* Confusion Matrix */}
        <div style={{ backgroundColor: '#111927', padding: '24px', borderRadius: '20px', border: '2px solid rgba(255,255,255,0.1)', flex: 1 }}>
          <h3 style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 900, fontFamily: 'Orbitron', margin: '0 0 16px' }}>
            CONFUSION MATRIX
          </h3>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <div style={{ fontSize: '0.6rem', color: '#6B7280', writingMode: 'vertical-rl', transform: 'rotate(180deg)', fontWeight: 700 }}>
              ACTUAL
            </div>
            <div>
              <div style={{ display: 'flex', gap: '4px', marginBottom: '4px', marginLeft: '60px' }}>
                {classLabels.map(l => (
                  <div key={l} style={{ width: '60px', textAlign: 'center', color: '#6B7280', fontSize: '0.6rem', fontWeight: 700 }}>{l}</div>
                ))}
              </div>
              {confusionMatrix.map((row, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '4px' }}>
                  <span style={{ width: '56px', color: '#6B7280', fontSize: '0.6rem', fontWeight: 700, textAlign: 'right' }}>{classLabels[i]}</span>
                  {row.map((val, j) => {
              ...
              <div style={{ textAlign: 'center', color: '#6B7280', fontSize: '0.6rem', fontWeight: 700, marginTop: '8px', marginLeft: '60px' }}>
                PREDICTED
              </div>
            </div>
          </div>
...
function DatasetsTab() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
        {DATASETS.map(ds => (
          <div key={ds.name} style={{ backgroundColor: '#111927', padding: '24px', borderRadius: '20px', border: '2px solid rgba(255,255,255,0.1)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
              <Database size={16} style={{ color: '#00F0FF' }} />
              <h3 style={{ color: '#fff', fontSize: '0.9rem', fontWeight: 900, margin: 0 }}>{ds.name}</h3>
              <span style={{ color: '#6B7280', fontSize: '0.65rem', marginLeft: 'auto' }}>{ds.year}</span>
            </div>
            <p style={{ color: '#9CA3AF', fontSize: '0.75rem', margin: '0 0 12px' }}>{ds.fullName}</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '12px' }}>
              <div style={{ padding: '8px', borderRadius: '8px', backgroundColor: 'rgba(0,0,0,0.3)' }}>
                <span style={{ color: '#6B7280', fontSize: '0.6rem' }}>Subjects</span>
                <div style={{ color: '#00FF66', fontWeight: 900, fontSize: '1.1rem', fontFamily: 'monospace' }}>{ds.subjects}</div>
              </div>
              <div style={{ padding: '8px', borderRadius: '8px', backgroundColor: 'rgba(0,0,0,0.3)' }}>
                <span style={{ color: '#6B7280', fontSize: '0.6rem' }}>Frames</span>
                <div style={{ color: '#00F0FF', fontWeight: 900, fontSize: '1.1rem', fontFamily: 'monospace' }}>{(ds.totalFrames / 1000).toFixed(0)}K</div>
              </div>
            </div>
            <div style={{ marginBottom: '8px' }}>
              <span style={{ color: '#6B7280', fontSize: '0.6rem', fontWeight: 700 }}>CLASSES:</span>
            ...
            <div style={{ marginBottom: '8px' }}>
              <span style={{ color: '#6B7280', fontSize: '0.6rem', fontWeight: 700 }}>CONDITIONS:</span>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '4px' }}>
                {ds.conditions.map(c => (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
          {[
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              {i < 7 && <span style={{ color: '#6B7280' }}>→</span>}
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1 }}>
          ...
          <thead>
            <tr>
              {['Method', 'Year', 'Dataset', 'Accuracy', 'F1', 'Approach'].map(h => (
                <th key={h} style={{ color: '#6B7280', fontSize: '0.65rem', fontWeight: 700, padding: '10px', textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {BENCHMARK_COMPARISONS.filter(b => b.accuracy > 0).map((b, i) => (
              <tr key={i} style={{ backgroundColor: b.method.includes('Ours') ? 'rgba(0,240,255,0.05)' : 'transparent' }}>
                ...
                <td style={{ color: '#9CA3AF', fontSize: '0.75rem', padding: '12px 10px' }}>{b.year}</td>
                <td style={{ color: '#9CA3AF', fontSize: '0.75rem', padding: '12px 10px' }}>{b.dataset}</td>
                ...
                <td style={{ color: '#6B7280', fontSize: '0.65rem', padding: '12px 10px', maxWidth: '250px' }}>{b.approach}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
        <div style={{ backgroundColor: '#111927', padding: '20px', borderRadius: '16px', border: '2px solid rgba(0,255,102,0.2)' }}>
          <TrendingUp size={20} style={{ color: '#00FF66', marginBottom: '12px' }} />
          <h4 style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 700, margin: '0 0 8px' }}>Edge-Deployable</h4>
        ...
        <div style={{ backgroundColor: '#111927', padding: '20px', borderRadius: '16px', border: '2px solid rgba(0,240,255,0.2)' }}>
          <Layers size={20} style={{ color: '#00F0FF', marginBottom: '12px' }} />
          <h4 style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 700, margin: '0 0 8px' }}>Multi-Signal Fusion</h4>
          <p style={{ color: '#9CA3AF', fontSize: '0.7rem', margin: 0 }}>
            7 physiological signals vs. single-metric methods. Reduces false positives by 67% vs. EAR-only.
          </p>
        </div>
        <div style={{ backgroundColor: '#111927', padding: '20px', borderRadius: '16px', border: '2px solid rgba(255,230,0,0.2)' }}>
  ...
  return (
    <div style={{ display: 'flex', gap: '24px', flex: 1 }}>
      ...
      <div style={{ width: '50%', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        <div style={{ backgroundColor: '#111927', padding: '24px', borderRadius: '20px', border: '2px solid rgba(255,140,0,0.3)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
            <XCircle size={18} style={{ color: '#FF8C00' }} />
            <h3 style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 900, fontFamily: 'Orbitron', margin: 0 }}>FALSE POSITIVES</h3>
          </div>
          <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
            <div style={{ flex: 1, padding: '12px', borderRadius: '10px', backgroundColor: 'rgba(255,140,0,0.1)' }}>
              <span style={{ color: '#6B7280', fontSize: '0.6rem' }}>Raw FP Rate</span>
            ...
            <div style={{ flex: 1, padding: '12px', borderRadius: '10px', backgroundColor: 'rgba(0,255,102,0.1)' }}>
              <span style={{ color: '#6B7280', fontSize: '0.6rem' }}>After Mitigation</span>
            ...
            {FALSE_POSITIVE_ANALYSIS.sources.map((fp, i) => (
              <div key={i} style={{ padding: '12px', borderRadius: '10px', backgroundColor: 'rgba(0,0,0,0.3)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                  <span style={{ color: '#fff', fontSize: '0.7rem', fontWeight: 700 }}>{fp.cause}</span>
      ...
      {/* False Negatives */}
      <div style={{ width: '50%', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        <div style={{ backgroundColor: '#111927', padding: '24px', borderRadius: '20px', border: '2px solid rgba(255,42,42,0.3)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
            <AlertTriangle size={18} style={{ color: '#FF2A2A' }} />
            <h3 style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 900, fontFamily: 'Orbitron', margin: 0 }}>FALSE NEGATIVES</h3>
          </div>
          <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
            <div style={{ flex: 1, padding: '12px', borderRadius: '10px', backgroundColor: 'rgba(255,42,42,0.1)' }}>
              <span style={{ color: '#6B7280', fontSize: '0.6rem' }}>Raw FN Rate</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {FALSE_NEGATIVE_ANALYSIS.sources.map((fn, i) => (
              <div key={i} style={{ padding: '12px', borderRadius: '10px', backgroundColor: 'rgba(0,0,0,0.3)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                  <span style={{ color: '#fff', fontSize: '0.7rem', fontWeight: 700 }}>{fn.cause}</span>
        ...
        {/* Summary */}
        <div style={{ backgroundColor: '#111927', padding: '20px', borderRadius: '16px', border: '2px solid rgba(255,255,255,0.1)' }}>
          <h4 style={{ color: '#fff', fontSize: '0.75rem', fontWeight: 700, margin: '0 0 12px' }}>Key Mitigations</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {[
